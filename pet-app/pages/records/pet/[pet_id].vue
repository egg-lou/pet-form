...
const fetchServiceInstances = async () => {
  const response = await serviceInstance.getPetHistories(pet_id, dateRange.value.start_date, dateRange.value.end_date);
...
watch(() => [dateStore.start, dateStore.end], async ([newStart, newEnd]) => {
    dateRange.value = {
        start_date: newStart,
        end_date: newEnd
    }
...
const handleDelete = async(service_instance_id:string) => {
  await serviceInstance.deleteServiceInstance(service_instance_id)
      .then(() => {
        toast({
...
<template>
    <div class="main px-10 md:px-20 ">
        <Card>
            <div class="h-[87vh] p-10 w-full flex flex-col gap-5">
                ...
                <div class="flex gap-10 px-10 py-3">
                  ...
                  <div class="flex flex-col gap-4">
                    <p>
                      ...
                      {{ pet?.pet_birth_date ? df.format(new Date(pet?.pet_birth_date)) : 'N/A'}}
                    </p>
                    <p>
              ...
              <div class="border-2 p-5">
                ...
                <ScrollArea class="h-[30rem] w-full px-10 py-1">
                  <h3 v-if="serviceInstances.length === 0"  class="text-center">
                    No records found.
                  </h3>
                  <Table v-if="serviceInstances.length > 0">
                    ...
                    <TableBody>
                      <TableRow v-for="service in serviceInstances" :key="service.service_instance_id">
                        <TableCell>{{service.service_instance_id}}</TableCell>
                        <TableCell>{{service.service_type.join(', ')}}</TableCell>
                        <TableCell>{{df.format(new Date(service.service_date))}}</TableCell>
                        <TableCell>{{service.requires_followup ? 'Yes' : 'No'}}</TableCell>
                        <TableCell>{{service.followup_date ? df.format(new Date(service.followup_date)) : 'N/A'}}</TableCell>
                        <TableCell>
                          <div class="flex gap-3">
                            <nuxt-link :to="`/records/pet/service_instance/${service.service_instance_id}`">
                            <Button>View</Button>
                            </nuxt-link>
                            <AlertDialog>
                              <AlertDialogTrigger>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction @click="handleDelete(service.service_instance_id)">Delete</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
