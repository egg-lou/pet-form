...
const ownerSchema = toTypedSchema(
  z.object({
    ...
    owner_address: z.string().min(10).max(15)
  })
);
...
const onSubmit = handleSubmit((values) => {
  ownerService
      .updateOwner(values, props.owner_id).then(() => {
        toast({
  ...
  <template>
    <Dialog>
      <DialogTrigger>
        <Button variant="default" @click="fetchOwnerAndPets" class="dark:text-accent-foreground">Edit</Button>
      </DialogTrigger>
      <DialogContent>
    ...
    <form
      ...
      <div class="w-full flex justify-end">
        <DialogClose as-child>
          <Button type="submit" variant="default" class="dark:text-accent-foreground" @click="onSubmit">Update</Button>
        </DialogClose>
      </div>
    </form>
      </DialogContent>
